...
const CHOICES: Record<string, string[]> = {
  tsundere: ["오늘 힘들었어...", "잘 지내고 있어?", "내일도 같이 해줄거야?", "응원해줘서 고마워."],
  genki:    ["오늘 정말 열심히 했어!", "같이 있으니 좋다~", "힘들 때도 있는데 괜찮아?", "66일 같이 완주하자!"],
  intellectual: ["오늘 습관 실천 완료!", "더 효율적인 방법이 있을까?", "동기가 떨어질 때 어떻게 해?", "함께여서 든든해."],
};
...
export default function ChatScreen() {
  ...
  useEffect(() => {
    if (messages.length === 0 && character) {
      setMessages([{ id: 0, from: "character", text: character.greeting }]);
    }
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderBottom: "1px solid rgba(160,210,240,0.5)",
      ...
      >
        ...
        <div className="ml-1">
          ...
          <div className="text-[10px]" style={{ color: "#7a9bb5" }}>{character.title}</div>
        </div>
      </div>
              ...
              style={
                msg.from === "user"
                  ...
                  : {
                      ...
                      boxShadow: "0 2px 8px rgba(90,150,200,0.08)",
                    }
              }
            >
              {msg.text}
              ...
              <div className="flex gap-1">
                {[0, 1, 2].map((i) => (
                  <div
                    ...
                    style={{ background: character.color, animation: `sparkle-twinkle 1s ease-in-out ${i * 0.25}s infinite` }}
                  />
                ))}
              </div>
            </div>
      ...
      {/* 선택지 버튼 */}
      <div
        className="px-4 pb-20 pt-3"
        style={{ borderTop: "1px solid rgba(160,210,240,0.45)", background: "rgba(255,255,255,0.80)" }}
      >
