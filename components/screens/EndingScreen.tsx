...
const ENDING_DATA = {
  best: {
    title: "베스트 엔딩", subtitle: "BEST ENDING", grade: "S",
    gradeColor: "#f0a020", bgGlow: "rgba(240,160,32,0.15)", icon: "♛",
  ...
  normal: {
    title: "노멀 엔딩", subtitle: "NORMAL ENDING", grade: "A",
    gradeColor: "#3a90d4", bgGlow: "rgba(58,144,212,0.12)", icon: "✦",
  ...
  bad: {
    title: "배드 엔딩", subtitle: "BAD ENDING", grade: "C",
    gradeColor: "#f472b6", bgGlow: "rgba(244,114,182,0.08)", icon: "✧",
...
const PANEL = { background: "rgba(255,255,255,0.90)", border: "1px solid rgba(160,210,240,0.55)", boxShadow: "0 2px 12px rgba(90,150,200,0.10)", borderRadius: "16px" };
...
export default function EndingScreen() {
  ...
  const handleNextLover = () => { if (loverLine < loverLines.length - 1) setLoverLine(loverLine + 1); };
    ...
    return (
      <div className="relative w-full h-full flex flex-col overflow-hidden" style={{ background: BG }}>
        ...
        <div
          ...
          style={{ background: `radial-gradient(ellipse 70% 50% at 50% 30%, ${character?.color ?? "#4aacef"}20 0%, transparent 70%)` }}
        />
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 z-10 px-6">
          ...
          {loverLine < loverLines.length - 1 ? (
            <GameButton onClick={handleNextLover} variant="secondary">다음 ▶</GameButton>
          ) : (
            <div className="flex flex-col gap-3 w-full max-w-xs">
              <GameButton fullWidth onClick={handleRestart}>새로운 여정 시작하기</GameButton>
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center overflow-hidden" style={{ background: BG }}>
      ...
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(ellipse 80% 60% at 50% 20%, ${ending.bgGlow} 0%, transparent 65%)` }}
      />
      ...
      <div className="flex flex-col items-center gap-5 px-8 pt-12 pb-8 z-10 w-full animate-slide-up overflow-y-auto hide-scrollbar">
        ...
        <div
          ...
          style={{
            background: `${ending.gradeColor}18`,
        ...
        <div className="text-center">
          <p className="text-xs tracking-[0.4em] uppercase" style={{ color: "#7a9bb5" }}>{ending.subtitle}</p>
          <h2 className="text-2xl font-black mt-1" style={{ color: ending.gradeColor }}>{ending.title}</h2>
        </div>
        ...
        {/* 대사 */}
        <div className="px-5 py-4 w-full max-w-xs text-center" style={{ ...PANEL, border: `1px solid ${character?.color ?? "#3a90d4"}33` }}>
          <p className="text-sm leading-7 italic" style={{ color: "#1a3a5c" }}>"{dialogue}"</p>
        </div>

        <p className="text-sm text-center leading-7 whitespace-pre-line" style={{ color: "#7a9bb5" }}>{ending.description}</p>

        {/* 최종 스탯 */}
        <div className="w-full max-w-xs p-4" style={PANEL}>
          <h3 className="text-xs tracking-widest uppercase mb-3 text-center" style={{ color: "#7a9bb5" }}>최종 기록</h3>
          <div className="grid grid-cols-3 gap-2 text-center">
            {[
              { value: completedDays.length, label: "완료일" },
              { value: streak,               label: "최고 스트릭" },
              { value: affection,            label: "호감도" },
            ].map((s) => (
              <div key={s.label}>
                <div className="font-black text-lg" style={{ color: ending.gradeColor }}>{s.value}</div>
                <div className="text-[10px]" style={{ color: "#7a9bb5" }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 w-full max-w-xs pb-4">
          ...
          <GameButton fullWidth variant={isBestEnding ? "ghost" : "primary"} onClick={handleRestart}>
            새로운 여정 시작하기
          </GameButton>
        </div>
      </div>
